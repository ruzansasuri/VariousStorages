 ...
 * Version: $Id StorageOrder.java, 10/03/2016$
 *
 */
public class StorageOrder<T extends Comparable<T>> implements StorageInterface<T>
{
    private Node<T> head;
    ...
    public void insert(T val)
    {
        ...
        {
            Node<T> trav = head;//  To traverse along the list
            if(val.compareTo(trav.getValue()) < 0)
            {
                head = new Node<T>(val,trav);
                return;
            }
            while(trav.hasNext())
            {
                Node<T> compare = trav.getNext();
                if(val.compareTo(compare.getValue()) < 0)
                {
                    trav.setNext(new Node<T>());
                    trav = trav.getNext();
                    trav.setNext(compare);
                    trav.setValue(val);
                    break;
                }
                trav = compare;
    ...
    {
        Node<T> trav = head; // To traverse along our list
